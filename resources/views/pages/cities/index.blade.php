        ...
        <div class="container">
            ...
            <div class="card gutter-b">
                <div class="card-header flex-wrap border-0 pt-2 pb-0">
                    <h3 class="card-label alert alert-success text-center"> المدن </h3>
                    <div class="card-title">
                       @if(Auth::user()->hasDirectPermission('اضافة ادمن'))
                        <a href="{{route('cities.create')}}"class="btn btn-primary">أضافة مدينة </a>
                        @endif
                    </div>
                </div>
                <div class="card-body">
                              ...
                              <td>
                                <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown"
                                        ...
                                        @if(Auth::user()->hasDirectPermission('تعديل ادمن'))
                    										<a class="btn btn-success" href="{{route('cities.edit', $city->id)}}"><i class="fa fa-edit"></i> تعديل</a>
                                        @endif
                                        
                                        @if(Auth::user()->hasDirectPermission('حذف ادمن'))
                    										<a class="btn btn-danger" data-toggle="modal" href="#myModal-{{$city->id}}"><i class="fa fa-trash"></i> مسح</a>
                                        @endif
                                      </div>
                    									<div class="modal fade" id="myModal-{{$city->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    										...
                    										<form role="form" action="{{route('cities.destroy', $city->id)}}" method="POST">
