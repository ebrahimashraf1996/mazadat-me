    ...
    <div class="d-flex flex-column-fluid">
        ...
        <div class="container">
            ...
            <div class="card card-custom gutter-b">
                <h3 class="card-label text-center alert alert-success"> المجموعات </h3>
              
                <div style="padding-right:5px">
                  <div class="card-title">
                    @if(Auth::user()->hasDirectPermission('اضافة ادمن'))
                    <a href="{{route('roles.create')}}"class="btn btn-primary title-page"> أضافة مجموعة</a>
                    @endif
                  </div>
                </div>
                <hr>
                              ...
                              <td>
                                <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown"
                                        ...
                                        @if(Auth::user()->hasDirectPermission('تعديل ادمن'))
                    										<a class="btn btn-success" href="{{route('roles.edit', $role->id)}}"><i class="fa fa-edit"></i> تعديل</a>
                                        @endif

                                        @if(Auth::user()->hasDirectPermission('حذف ادمن'))
                    										<a class="btn btn-danger" data-toggle="modal" href="#myModal-{{$role->id}}"><i class="fa fa-trash"></i>حذف</a>
                                        @endif
                    									</div>
                    									<div class="modal fade" id="myModal-{{$role->id}}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                    										...
                    										<div class="modal-header">
                    											<h5 class="modal-title" id="exampleModalLabel{{$role->id}}">مسح المجموعة</h5>
                    											<button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    											</button>
                    										</div>
                    										<div class="modal-body">
                    										<form role="form" action="{{route('roles.destroy', $role->id)}}" method="POST">
